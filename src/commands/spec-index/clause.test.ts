...
describe("suggestIds", () => {
	it("同じ AREA の条項を候補に出す", () => {
		expect(
			suggestIds(["K-CORE-TYPE", "999"].join("-"), [
				"K-CORE-TYPE-001",
				"K-CORE-DEF-001",
			]),
	...
	it("AREA が一致しなければ前方一致で拾う", () => {
		expect(
			suggestIds(["K-CORE-XXXX", "001"].join("-"), ["K-CORE-TYPE-001"]),
		).toEqual(["K-CORE-TYPE-001"]);
	});
});
...
describe("renderIndex", () => {
	...
	it("ファイルごとに分け、条項を行番号順に並べる", () => {
		...
		expect(output.indexOf("K-CORE-DEF-001")).toBeLessThan(
			output.indexOf("K-CORE-DEF-002"),
		);
	});
	...
	it("withdrawn の件数を数える", () => {
		expect(
			renderIndex([
				clause({}),
				clause({ id: "K-CORE-DEF-002", status: "withdrawn" }),
			]),

...
describe("planIndexes", () => {
	it("実体を持つ側ごとに索引を作り、根の stub は載せない", async () => {
		...
		const root = writes.find(
			(w) => w.path === join(repo, "docs", "task", "INDEX.md"),
		);
	...
	it("実体が0件になった索引は削除の対象になる", async () => {
		...
		expect(removals.map((p) => relative(repo, p))).toEqual([
			join("docs", "task", "INDEX.md"),
		]);
	});

	it("このリポジトリの索引が生成物と一致している(check:task-index と同じ判定)", async () => {
		const packages = await discoverPackages(REPO_ROOT);
		const { writes, removals } = planIndexes(REPO_ROOT, "task", packages);

		expect(removals).toEqual([]);
		expect(writes.length).toBeGreaterThan(0);
		for (const w of writes) {
			expect(existsSync(w.path)).toBe(true);
			expect(readFileSync(w.path, "utf8")).toBe(w.content);
		}
	});
});
